...
//y si fue visitado ya se le seteo dis[padre] + 1. Pero queda bien
	...
	while ( !es_vacia_cola(nodos_pendientes) ) {
		...
		vecinos = copia_lista(adyacentes(nodo_actual, G));	// como mucho copia n-1 aristas en caso de que el nodo se conecte con todos los demas, < O(n)
	//podria ahorrarme este paso de copiar si itero con un puntero aux ??
		while ( !es_vacia_lista(vecinos) ) {	// < O(n)
			...
			remover(vecino_actual, vecinos); //siempre demora O(1) pues remueve el primer elemento
		}
		destruir_lista(vecinos);	//O(1) pues la lista sale vacia
	}
